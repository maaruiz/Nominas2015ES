...
def to_word(number, mi_moneda=None):
    ...
    cad = ' '.join(human_readable).replace('  ',' ').title().strip()
    if cad[0:len('un mil')] == 'Un Mil':
        cad = 'Mil' + cad[len('Un Mil'):]
    if num_decimals != ['00']:
        cad = cad + ' con ' + ' '.join(human_readable_decimals).replace('  ',' ').title().strip()
    return cad
